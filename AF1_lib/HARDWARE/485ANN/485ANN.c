...
/*
	   if((tempshuzhi>=70||dianya_zhi>=440||dianya_zhi<=340)&&alarm_lock==0)
	   	{   
                                        if(mybox.master==1)//Ö÷»ú·¢²¼Í¨ÖªÑÓÊ±ĞÅÏ¢
								  {
								       delay_time(80);
								  }   
		       ...
		       LIGHT(mystatus.work_status[0],mystatus.work_status[1]);//¹ıÎÂ³£Ì¬
		       alarm_lock=1;

	   }
	   
	/*	   if((dianya_zhi>=440||dianya_zhi<=340)&&mybox.master==1)//Èç¹ûÊÇ¹ıÑ¹»òÇ·Ñ¹²¢ÇÒ Èç¹ûÊÇÖ÷»ú½øÈë¸ÃÑ­»·
		   	{
                             while(1)
					{   gonglvyinshu();//¼ÆËã¹¦ÂÊ£¬µçÑ¹µçÁ÷ÓëÏÔÊ¾°´¼ü·Ö¿ª
                                        temperature();
...
//tempshuzhi=K_BT;
K_BT=TR[15];//Ğ´ËÀ±ä±È´¦£¬ÓÃÓÚ·Ç×Ô¶¯ÅĞ¶Ï±ä±È£¬Èç¹û²ÉÓÃ×Ô¶¯»ñÈ¡±ä±ÈĞèÒª×¢µô¸Ã¾ä»°
if(RT_FLAG==2)
{
gonglvyinshu();//¼ÆËã¹¦ÂÊ£¬µçÑ¹µçÁ÷ÓëÏÔÊ¾°´¼ü·Ö¿ª
